...
authRouter.post('/registration', requestHandler(async (req) => {
    const input = RegistrationInputSchema.parse(req.body);

    const user = await userService.register(input);
    const token = jwtService.create({ id: user.id, email: user.email, role: user.role });
...
authRouter.post('/login', requestHandler(async (req) => {
