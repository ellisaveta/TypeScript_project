...
export function isCurrentlyLoggedUserTheOwner(res: Response, userId: number) {
    if (userIdFromLocals(res) !== userId) {
        throw new UnauthorizedError('This operation can be performed only by the owner!');
    }
    return true;
}
...
userRouter.delete('/:id', authMiddleware, requestHandler(async (req, res) => {
    const { id } = IdInputSchema.parse(req.params);

    await findUserWithThatId(id);
    isCurrentlyLoggedUserTheOwner(res, id); // or admin
