...
import { z } from "zod";
import { ModifyReviewInputSchema, ReviewService } from "../services/review-service";
...
reviewRouter.patch('/:id', authMiddleware, requestHandler(async (req, res) => {
    const { id } = IdInputSchema.parse(req.params);
    const review = await findReviewWithThatId(id);
    await isCurrentlyLoggedUserTheOwner(res, review.userId);
...
reviewRouter.delete('/:id', authMiddleware, requestHandler(async (req, res) => {
    const { id } = IdInputSchema.parse(req.params);
    const review = await findReviewWithThatId(id);
    await isCurrentlyLoggedUserTheOwner(res, review.userId); // or admin
