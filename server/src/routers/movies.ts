...
import { authMiddleware, userIdFromLocals } from '../middlewares/auth-middleware';
import { requestHandler, ResponseWithCode } from '../middlewares/request-handler';
import { AddMovieInputSchema, ModifyMovieInputSchema, MovieService } from '../services/movie-service';
import { LikeService } from '../services/like-service';
import { CommentService } from '../services/comment-service';
import { AddReviewInputSchema, ReviewService } from '../services/review-service';
...
movieRouter.get('/', authMiddleware, requestHandler(async (req) => {
    const { page, size, searchText } = GetMoviesInputSchema.parse(req.query);
    const { results, total } = await movieService.listAllMovies(page, size, searchText);
    return { movies: movieTransformer.transformArray(results), total };
}));

movieRouter.get('/rating', requestHandler(async (req) => {
    const { page, size } = GetMoviesInputSchema.parse(req.query);
    const { results, total } = await movieService.moviesOrderedByAverageRating(page, size);
    return { movies: movieTransformer.transformArray(results), total };
}));

movieRouter.get('/:id', requestHandler(async (req) => {
...
movieRouter.get('/:id/likes', requestHandler(async (req) => {
    const { id } = IdInputSchema.parse(req.params);
...
movieRouter.get('/:id/comments', requestHandler(async (req) => {
    const { id } = IdInputSchema.parse(req.params);
...
movieRouter.get('/:id/reviews', requestHandler(async (req) => {
    const { id } = IdInputSchema.parse(req.params);
...
movieRouter.get('/:id/info', requestHandler(async (req) => {
    const { id } = IdInputSchema.parse(req.params);
...
movieRouter.post('/', authMiddleware, requestHandler(async (req) => {
...
movieRouter.post('/:id/like', authMiddleware, requestHandler(async (req, res) => {
...
movieRouter.post('/:id/comment', authMiddleware, requestHandler(async (req, res) => {
    const { id: movieId } = IdInputSchema.parse(req.params);
...
movieRouter.post('/:id/review', authMiddleware, requestHandler(async (req, res) => {
    const { id: movieId } = IdInputSchema.parse(req.params);
...
movieRouter.patch('/:id', authMiddleware, requestHandler(async (req) => {
    const { id } = IdInputSchema.parse(req.params);
    ...
    if (!modifiedMovie) {
        throw new NotFoundError(`Nothing was modified! There is no movie with id: ${id}!`);
    }

    return { movie: movieTransformer.transform(modifiedMovie) };
}));

movieRouter.delete('/:id', authMiddleware, requestHandler(async (req) => {
    const { id } = IdInputSchema.parse(req.params);
    await findMovieWithThatId(id);
