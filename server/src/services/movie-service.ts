...
export class MovieService {
    async add(movie: AddMovieInput) {
        return await MovieModel.query()
            .insertAndFetch({
                ...
                mainStar: movie.mainStar, description: movie.description, releaseDate: movie.releaseDate
            });
    }
    ...
    async listAllMovies(pageNumber: number, size: number, searchText?: string) {
        return await MovieModel.query()
            .where('title', 'ilike', `%${searchText?.replace(/%/g, '\%')?.replace(/_/g, '\_')}%`)
            .orderBy('title', 'ASC').page(pageNumber - 1, size);
    }

    async moviesOrderedByAverageRating(pageNumber: number, size: number) {
        return await MovieModel.query().leftJoin('reviews', 'movies.id', '=', 'reviews.movieId').select('movies.*')
            .avg('reviews.rating as rate').groupBy('movies.id')
    ...
    async getMovieWithComments(id: number) {
        return await MovieModel.query().findById(id).withGraphFetched('comments.[user]');
    }

    async getMovieWithReviews(id: number) {
        return await MovieModel.query().findById(id).withGraphFetched('reviews.[user]');
    }

    async getMovieWithInfo(id: number) {
        return await MovieModel.query().findById(id).withGraphFetched('[likedBy, comments.[user], reviews.[user]]');
    }
    ...
    async update(id: number, data: ModifyMovieInput) {
        ...
        return await movie?.$query().patchAndFetch({
            title: data?.title, director: data?.director, mainStar: data?.mainStar,
            description: data?.description, releaseDate: data?.releaseDate, poster: data?.poster
        });
    }
}