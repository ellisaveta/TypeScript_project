...
describe("MovieService", () => {
  describe("#add", () => {
    it("adds movie", async () => {
      ...
      const added = await movieService.add({
        ...
        poster:
          "https://www.shutterstock.com/image-vector/picture-vector-icon-no-image-260nw-1350441335.jpg",
      });

      const movie = await MovieModel.query().findOne({
        title: "Titanic",
        director: "James Cameron",
      });
  ...
  describe("#searchMoviesByDirector", () => {
    ...
    it("does not find movie with unexisting director", async () => {
      ...
      const movie = await movieService.searchMoviesByDirector(
        "James Cameron 15"
      );
