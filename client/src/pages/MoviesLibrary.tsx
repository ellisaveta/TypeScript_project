...
export function MoviesLibrary() {
    ...
    const { data, loading, error } = useAsync(() => moviesService.getAllMovies(Number(pageQuery), PAGE_SIZE, query), [pageQuery, query]);
    ...
    const onNextPageRequest = useCallback(() => {
        if (data?.total) {
            ...
            if ((page > pagesNum) || (page === pagesNum && data.total % PAGE_SIZE === 0)) {
                return;
            }
        }
        page++;
    ...
    return (
        <>
            <div className={classes.flex}>
                ...
                <Input className={classes.search} type='search' value={query}
                    onChange={(e) => { onSearchInputChange(e) }} placeholder='Search' />
                <Button variant='secondary' className={classes.viewButton} onClick={onViewChange}>Change view</Button>
            </div>
            <div>
                {loading && <>Loading...</>}
                {!!error && <span className={classes.errorLabel}>Something went wrong</span>}
                {data?.movies &&
                    <>
                        {isGrid ? <GridMoviesView movies={data.movies} /> : <ListMoviesView movies={data.movies} />}
                        {data.total > 6 ?
                            <div className={classes.pagination}>
                                <Button onClick={onPreviousPageRequest}>&#8882;</Button>
                                <p>{pageQuery}</p>
                                <Button onClick={onNextPageRequest}>&#8883;</Button>
                            </div> :
                            data.total === 0 ? <h3>No movies to show!</h3> : null}
                    </>}
            </div>
        </>
    );
