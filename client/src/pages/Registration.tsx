...
export function Registration() {
    ...
    const { loading, error, trigger } = useAsyncAction(async (event: FormEvent) => {
    ...
    return (
        <FormLayout>
            ...
            <form className={classes.form} onSubmit={trigger}>
                ...
                {!!error && error instanceof EmailAlreadyExists && <span className={classes.errorMessage}>{error.message}</span>}
                {!!error && !(error instanceof HttpError) && <span className={classes.errorMessage}>Something went wrong</span>}
                <Button variant='accent' disabled={loading} type="submit" className={classes.register}>
