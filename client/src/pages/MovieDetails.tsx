...
export function MovieDetails() {
    const { id } = useParams();
    const { data: movie, loading, error, reload } = useAsync(() => moviesService.getById(Number(id)), [id]);
    if (loading && !movie) {
    ...
    return (movie ?
        ...
        : error ? <span className={classes.errorLabel}>Something went wrong!</span> : null
    );
}